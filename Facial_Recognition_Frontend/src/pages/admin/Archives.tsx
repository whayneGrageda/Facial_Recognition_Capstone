...
import type { CollegeUser, ShsUser, FacultyUser, Guest, Moderator } from '../../types';
import './Archives.css';

type ArchivedUser = (CollegeUser | ShsUser | FacultyUser | Guest | Moderator) & {
  archived_date?: string;
  archived_at?: string;
};

type UserType = 'college' | 'shs' | 'faculty' | 'guests' | 'moderators';
...
const Archives = () => {
  ...
  const tabs = [
    ...
    { id: 'moderators' as UserType, label: 'Moderators', count: 0 },
  ];
  ...
  const fetchArchivedUsers = async () => {
    ...
    try {
      ...
      if (activeTab === 'college') {
        ...
        const response = await moderatorService.getArchived(recordsPerPage, offset, filters);
        data = response.moderators || [];
        total = response.totalCount || 0;
      }
  ...
  const handleRestore = async (userId: number) => {
    ...
    try {
      if (activeTab === 'college') {
        ...
        await moderatorService.restore(userId);
      }
  ...
  const handleDelete = async (userId: number) => {
    ...
    try {
      if (activeTab === 'college') {
        await userService.college.permanentDelete(userId);
      } else if (activeTab === 'shs') {
        await userService.shs.permanentDelete(userId);
      } else if (activeTab === 'faculty') {
        await userService.faculty.permanentDelete(userId);
      } else if (activeTab === 'guests') {
        await guestService.permanentDelete(userId);
      } else if (activeTab === 'moderators') {
        await moderatorService.permanentDelete(userId);
      }
  ...
  const handleBulkRestore = async () => {
    ...
    try {
      if (activeTab === 'college') {
        ...
        await moderatorService.bulkRestore(selectedUsers);
      }
      ...
      <div className="archives-card">
        ...
        <div className="filters-container">
          <div className="filters-grid">
            ...
            {activeTab !== 'moderators' && activeTab !== 'guests' && (
              <div className="filter-group">
              ...
              {loading ? (
                ...
                archivedUsers.map((user) => (
                  <tr key={user.id}>
                    ...
                    <td>{user.id}</td>
                    <td>{'name' in user ? user.name : 'username' in user ? user.username : 'N/A'}</td>
